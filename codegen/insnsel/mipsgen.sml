...
fun codegen (frame: Frame.frame) (stm: Tree.stm) : Assem.instr list = 
    ...
    fun pushStackForCall (funName, numArgs) = if isLibraryCall(funName) andalso numArgs > 4 then emit(A.OPER{assem="add $sp, $sp, -"^printInt((numArgs-4)*4)^"\n", src=[], dst=[], jump=NONE})
      else (if numArgs > 4 then emit(A.OPER{assem="add $sp, $sp, -"^printInt((numArgs-4)*4)^"\n", src=[], dst=[], jump=NONE})
        else (if isLibraryCall(funName) then () else emit(A.OPER{assem="add $sp, $sp, -4\n", src=[], dst=[], jump=NONE})))
    
    fun pullStackAfterCall (funName, numArgs) = if isLibraryCall(funName) andalso numArgs > 4 then emit(A.OPER{assem="add $sp, $sp, "^printInt((numArgs-4)*4)^"\n", src=[], dst=[], jump=NONE})
      else (if numArgs > 4 then emit(A.OPER{assem="add $sp, $sp, "^printInt((numArgs-4)*4)^"\n", src=[], dst=[], jump=NONE})
        else (if isLibraryCall(funName) then () else emit(A.OPER{assem="add $sp, $sp, 4\n", src=[], dst=[], jump=NONE})))
    ...
    fun saveCallerSavedRegs() = (
      ...
      emit(A.OPER{assem="add $sp, $sp, -44\n", src=[], dst=[], jump=NONE});
    ...
    fun saveCalleeSavedRegs() = (
        ...
        emit(A.OPER{assem="add $sp, $sp, -"^printInt(4*(9+getCurrentFrameFormalsCount()))^"\n", src=[], dst=[], jump=NONE});
        saveSRegs(8);
    ...
    fun result(gen) =
      ...
      | munchStm(T.MOVE(T.MEM(e1), e2)) = 
            ...
            dst=[], jump=NONE})
      | munchStm(T.MOVE(T.TEMP i, T.CONST 0)) = emit(A.OPER{assem="move `d0, $0 \n",
        src=[], dst=[i], jump=NONE})
