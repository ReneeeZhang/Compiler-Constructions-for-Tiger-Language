...
struct 
  ...
  val base_venv =
    let val venv1 = S.enter(S.empty, S.symbol("print"), {access=(), ty=T.ARROW([T.STRING], T.UNIT)})
        val venv2 = S.enter(venv1, S.symbol("flush"), {access=(), ty=T.ARROW([], T.UNIT)})
        ...
        val venv_final = S.enter(venv9, S.symbol("exit"), {access=(), ty=T.ARROW([T.INT], T.UNIT)})
    in
        venv_final
    end
